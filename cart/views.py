...
class CartItems(generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):       
        ...
        if product_id and quantity:
            ...
            # if cart.coupon:
            #     coupon = Cart.objects.get(id=cart.coupon)
            #     if cart.total < coupon.required_amount_to_use_coupon:
            #         cart.remove_coupon()
            #         messages.warning(self.request, "The coupon has been removed because the total is below {coupon.required_amount_to_use_coupon")
            #         return redirect('cart')
        if clear:
            cart.clear()
            return redirect('cart')
...
class AddCoupon(generic.View):
    def post(self, *args, **kwargs):
        ...
        if coupon.exists():
            ...
            if cart.total() < coupon.required_amount_to_use_coupon:
                messages.warning(self.request, "You have to shop at least {coupon.required_amount_to_use_coupon} to use this coupon code.")
                return redirect('cart')
